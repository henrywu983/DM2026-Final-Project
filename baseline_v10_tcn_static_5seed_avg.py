...
# ───────────────────────── Multi-seed training helpers ─────────────────────────

def parse_seeds(seed_string: str) -> List[int]:
    """Parse comma-separated seeds, e.g. '42,43,44,45,46'."""
    seeds = []
    for s in seed_string.split(','):
        s = s.strip()
        if s:
            seeds.append(int(s))
    if not seeds:
        raise ValueError('No seeds provided. Example: --seeds 42,43,44,45,46')
    return seeds


def set_all_seeds(seed: int):
    """Set random seeds for reproducible model initialization and dataloader shuffle."""
...
def make_train_loader(train_ds, batch_size: int, seed: int):
    """Create a training DataLoader with seed-specific shuffling."""
...
def train_one_seed(
    ...
    """Train one V10 TCN+Static model and return the best model plus validation summary."""
    set_all_seeds(seed)

    train_loader = make_train_loader(train_ds, args.batch_size, seed)
    ...
    print(f'\n[Training seed {seed}] {args.epochs} epochs max, patience {args.patience}')
    for epoch in range(1, args.epochs + 1):
        ...
        if epochs_no_improve >= args.patience:
            print(f'  [early stop seed {seed}] no improvement for {args.patience} epochs')
            break

    print(f'\n[Best seed {seed}] val MAE = {best_val:.4f}')
    model.load_state_dict(best_state)
    return model, float(best_val), best_week_mae


def build_submission(sub, test_region_ids, raw_preds):
    """Build a submission DataFrame in sample_submission order."""
...
def main():
    ...
    parser.add_argument('--out', default='baseline_v10_tcn_static_5seed_avg.csv')
    ...
    parser.add_argument('--seeds', type=str, default='42,43,44,45,46',
                        help='Comma-separated seeds to train and average, e.g. 42,43,44,45,46')
    parser.add_argument('--save-seed-preds', action='store_true',
                        help='Also save one submission CSV per individual seed.')
    ...
    print(f'[V10 5-seed config] channels={args.channels}, n_blocks={args.n_blocks}, '
          f'static_dim={args.static_dim}, n_static={N_STATIC}, seeds={seeds}')

    # Load and preprocess data ONCE. All seeds share the same train/val/test tensors.
    print('\n[Loading raw data]')
    ...
    print(f'  train samples: {len(train_ds)}, val samples: {len(val_ds)}, val batches: {len(val_loader)}')
    ...
    # Train one model per seed and collect test predictions.
    seed_preds = []
    seed_summaries = []
    for seed in seeds:
        model, best_val, best_week_mae = train_one_seed(
            seed=seed,
            args=args,
            train_ds=train_ds,
        ...
        print(f'  seed {seed} test preds: mean {raw_preds.mean():.3f}, median {np.median(raw_preds):.3f}')

        seed_preds.append(raw_preds)
        seed_summaries.append((seed, best_val, best_week_mae))

        if args.save_seed_preds:
            seed_submission = build_submission(sub, test_region_ids, raw_preds)
            stem, ext = os.path.splitext(args.out)
            seed_out = os.path.join(args.base, f'{stem}_seed{seed}{ext or ".csv"}')
            seed_submission.to_csv(seed_out, index=False)
            print(f'  [Saved individual seed] {seed_out}')

        # Free GPU memory before training the next seed.
        del model
        if torch.cuda.is_available():
            torch.cuda.empty_cache()

    print('\n[Validation summary by seed]')
    for seed, best_val, week_mae in seed_summaries:
        print(f'  seed {seed}: best val {best_val:.4f}  '
              f'[w1 {week_mae[0]:.3f} w2 {week_mae[1]:.3f} w3 {week_mae[2]:.3f} '
              f'w4 {week_mae[3]:.3f} w5 {week_mae[4]:.3f}]')

    print('\n[Averaging seed predictions]')
    avg_preds = np.mean(np.stack(seed_preds, axis=0), axis=0)
    avg_preds = np.clip(avg_preds, 0.0, 5.0)
    print(f'  averaged preds: shape {avg_preds.shape}, mean {avg_preds.mean():.3f}, '
          f'median {np.median(avg_preds):.3f}')

    submission = build_submission(sub, test_region_ids, avg_preds)
    out_path = os.path.join(args.base, args.out)
    submission.to_csv(out_path, index=False)
    print(f'\n[Saved averaged submission] {out_path}')
    print('\nFirst 5 rows:')
